...
lz_info optimal_parsing (
	...
	if(match_length == 2) {
		if(match_offset <= cfg_short_limit) {
			...
			//XXX TODO if previous element is also match: cost++
...
unsigned find_matches(lz_context *ctx, unsigned window) {
...
void encode_match_8class (
	...
	if(length == 2 - 1) {
		...
		for (i = 0; i < 8; i++) {
			if (offset_prefix == prefix_order[i]) break;
		}
		offset_prefix = i;
	...
	output_bit(ctx, (offset_prefix >= 4));
...
void write_output(lz_context *ctx) {
	unsigned cursor;
...
//	unsigned output_end = ctx->output_end;
	...
	signed packed_size = ctx->file_size;
	signed last_match = 0;
	...
	for(cursor = ctx->output_begin; cursor < ctx->output_end; cursor += length) {
		...
		if (ctx->output) {
			...
			if(!ctx->overlap && stream_pos > dest_pos) {
				if(update) {
					ctx->end_pos = cursor;
					update = false;
				}
...
#			if USE_LITERAL_RUNS
			{
				...
				if(implicit_match) {
					...
					//is the next run a literal too? is the length of the current literal smaller then teh run limit and is the next element a literal of maximum runlength? If so, swap both, and if things continue, do so also on upcoming turns.
...
unsigned compress(lz_context* ctx, char* output_name, unsigned window) {
	...
	return find_matches(ctx, window);
}
...
//			cfg_per_page = true;
			...
			opt_addr = read_number(*++argv);
			--argc;
		} else if(argc >= 2 && !strcmp(*argv, "--depack-to")) {
			depack_to = read_number(*++argv);
			--argc;
		} else if(!strcmp(*argv, "--raw")) {
			ctx.output_type = OUTPUT_RAW;
...
//			argc -= 2;
			...
			cut_addr_first = read_number(*++argv);
			cut_addr_last = read_number(*++argv);
			if(cut_addr_first > cut_addr_last)
...
//			cfg_per_page = true;
	...
	if (!offset_lengths) parse_offset_lengths(&ctx, DEFAULT_LENGTHS);
...
//			fprintf(stderr, "warning: sliding-window used with a PRG file\n");
//		}
	} else {
		if (ctx.output_type == OUTPUT_SFX || ctx.output_type == OUTPUT_LEVEL || ctx.output_type == OUTPUT_BITFIRE) {
			fatal("sfx/level/bitfire not supported with raw binaries");
		}
	}
	...
	if(!output_name) {
		...
		output_name = malloc(name_length + 1 + sizeof extension);
	...
	cut_input_addr(&ctx, cut_addr_first, cut_addr_last, ctx.full_dict);
	...
	if (iterations > 0) {
		...
		//sprintf(lengths,"12/13/14/15:12/13/14/15");
	...
	// now go live and find perfect safety margin
	ctx.output = true;
	write_output(&ctx);
	...
	if (ctx.overlap && opt_addr < 0 && ctx.output_type != OUTPUT_SFX) {
		printf("overlap: %d bytes\n", ctx.margin);
	}

	printf("source size: $%04x (%d)\n", source_size, source_size);
	printf("packed size: $%04x (%d) %s ratio: %.1f%%\n", packed_size - write_tables * 24, packed_size - write_tables * 24, write_tables ? "(+24 byte tables)" : "", ((packed_size - write_tables * 24) * 100.0 / (ctx.output_end - ctx.output_begin)));

	//print info and calc addresses
	if(ctx.output_type == OUTPUT_LEVEL || ctx.output_type == OUTPUT_BITFIRE) {
		if (ctx.overlap) {
			if (opt_addr < 0) opt_addr = ctx.output_end - packed_size + ctx.margin;
		} else {
			if (opt_addr < 0) opt_addr = ctx.output_end - packed_size;
		}
		printf("source load: $%04x-$%04x\n", ctx.output_begin, ctx.output_end);
		printf("packed load: $%04x-$%04x\n", opt_addr, opt_addr + packed_size);
		if (opt_addr < ctx.output_begin) {
			fatal("compressed file is bigger than original");
		}
		if ((opt_addr > 0xd000 && opt_addr < 0xe000) || (opt_addr + packed_size > 0xd000 && opt_addr + packed_size < 0xe000)) {
		...
		if (!ctx.overlap) {
			if (ctx.output_end - ctx.end_pos > 255) {
				fprintf(stderr,"WARNING: more than one block of final binary blob, additional loading after depacking might fail.\n");
				if (ctx.exit_on_warn) exit(EXIT_FAILURE);
			}
//			printf("depack-end-addr: $%04x\n",ctx.end_pos);
		}
	} else if(ctx.output_type == OUTPUT_SFX) {
	...
	// manipulate the file and addresses in file
	if(ctx.output_type == OUTPUT_LEVEL || ctx.output_type == OUTPUT_BITFIRE) {
	...
	//manipulate depack destination @ offset 2 in file
	if (depack_to >= 0) {
		fseek(ctx.dst_file, 0x2, SEEK_SET);
		fputc(depack_to & 0xff, ctx.dst_file);
		fputc(depack_to >> 8, ctx.dst_file);
	}
	fclose(ctx.dst_file);
